...
def normalize(term: DBTerm, max_steps: int = 7) -> DBTerm:
    curr = term
    ...
    for name, decl in sorted_declarations:
        if db_equal(term, decl):
            if all_defined_symbols is not None and name in all_defined_symbols and name not in declared:
                continue
            return name
        # Bounded reduction for trivial assignment normalizations
        try:
            if db_equal(normalize(term, 20), normalize(decl, 20)):
                if all_defined_symbols is not None and name in all_defined_symbols and name not in declared:
                    continue
                return name
        except Exception:
