...
# Surface AST for Named Parsing Phase
...
def debruijn_to_str(
    ...
    # 1. Lookup in declared dictionary (ignoring symbols currently being defined)
    ...
    # 2. Resugar Church Integers (unless integer string matches target being defined)
    num = try_decode_church(term)
    if num is not None:
        if str(num) not in exclude_names:
            return str(num)
    ...
    if isinstance(term, DBVar):
        ...
        # Map free variables back to original identifiers
...
def verify_unit_idempotency(
    ...
    """Unit Assertion: Validates that resugaring an expression preserves normal-form semantics."""
    try:
        ...
        norm_orig = normalize(term)
        norm_parsed = normalize(parsed_db)

        if not db_equal(norm_orig, norm_parsed):
            print(
                f"\n❌ UNIT IDEMPOTENCY FAILED at {filename}:{line_no}\n"
                f"   Target: {target_names}\n"
                f"   Resugared: {resugared_str}\n"
                f"   Original Norm: {debruijn_to_str(norm_orig, declared_terms)}\n"
                f"   Parsed Norm:   {debruijn_to_str(norm_parsed, declared_terms)}",
                file=sys.stderr
            )
            sys.exit(1)
...
def execute_program(
        ...
        try:
            if action == "ASSERT":
                ...
                if not true_term or not db_equal(evaluated, true_term):
                    ...
                    print(f"   Got: {debruijn_to_str(evaluated, declared_terms)}", file=sys.stderr)
                    sys.exit(1)

                pretty_expr = debruijn_to_str(db_term, declared_terms)
                if enable_idempotency_check:
                    verify_unit_idempotency(db_term, pretty_expr, env, declared_terms, parser, line_no, filename)
                ...
                if enable_idempotency_check:
                    verify_unit_idempotency(db_left, pretty_left, env, declared_terms, parser, line_no, filename)
                    verify_unit_idempotency(db_right, pretty_right, env, declared_terms, parser, line_no, filename)
                ...
                pretty = debruijn_to_str(db_term, declared_terms, exclude_names=exclude_set)

                if enable_idempotency_check:
                    verify_unit_idempotency(db_term, pretty, env, declared_terms, parser, line_no, filename, target_names=target)
                ...
                pretty_in = debruijn_to_str(db_term, declared_terms)
                pretty_res = debruijn_to_str(evaluated, declared_terms)

                if enable_idempotency_check:
                    verify_unit_idempotency(db_term, pretty_in, env, declared_terms, parser, line_no, filename)
    ...
    # Integration Assertion: Re-evaluate generated code to verify pass-to-pass string stability
    if enable_idempotency_check:
        resugared_code = "\n".join(output_lines)
