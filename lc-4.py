...
def verify_unit_idempotency(
    ...
    """Unit Assertion: Validates that resugaring an expression preserves structural semantics."""
    try:
        ...
        # FIX: Directly compare the structural De Bruijn terms without normalizing them
        if not db_equal(term, parsed_db):
            print(
                f"\n❌ UNIT IDEMPOTENCY FAILED at {filename}:{line_no}\n"
                f"   Target: {target_names}\n"
                f"   Resugared: {resugared_str}\n"
                f"   Original: {debruijn_to_str(term, declared_terms)}\n"
                f"   Parsed:   {debruijn_to_str(parsed_db, declared_terms)}",
                file=sys.stderr
            )
            sys.exit(1)
